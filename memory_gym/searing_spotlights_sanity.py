...
class SearingSpotlightsSanityEnv(gym.Env):
    metadata = {
    ...
    def reset(self, seed = None, return_info = True, options = None):
        ...
        self.reset_params = SearingSpotlightsSanityEnv.process_reset_params(options)
        self.t = 0
        ...
        return {"visual_observation": vis_obs, "vector_observation": np.asarray([self.agent.rect.center[0] / self.screen_dim, self.agent.rect.center[1] / self.screen_dim])}, {}
    ...
    def step(self, action):
        ...
        return {"visual_observation": vis_obs, "vector_observation": np.asarray([self.agent.rect.center[0] / self.screen_dim, self.agent.rect.center[1] / self.screen_dim])}, reward, done, False, info
    ...
    def render(self):
        if self.render_mode is not None:
            if self.render_mode == "rgb_array":
                self.clock.tick(SearingSpotlightsSanityEnv.metadata["render_fps"])
...
def main():
    ...
    env = SearingSpotlightsSanityEnv(render_mode = "rgb_array")
    reset_params = {}
    seed = options.seed
